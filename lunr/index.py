...
class Index:
    ...
    def __init__(self, inverted_index: Dict[str, Dict], field_vectors: Dict[str, Vector],
                 token_set: TokenSet, fields: Collection[str], pipeline: Pipeline):
    ...
    def query(self, query: Union[Query, None] = None,
              callback: Union[QueryCallback, None] = None) -> List[Dict]:
    ...
    def load(cls, serialized_index: Union[str, SerializedIndex]) -> "Index":
        ...
        if not isinstance(serialized_index.get("fieldVectors"), list):
            raise BaseLunrException(
                "Serialized index has no list of field_vectors") from ValueError
        if not isinstance(serialized_index.get("invertedIndex"), list):
            raise BaseLunrException(
                "Serialized index has no inverted index") from ValueError
        if not isinstance(serialized_index.get("fields"), list):
            raise BaseLunrException(
                "Serialized index has no list of fields") from ValueError
